        ...
        elements={getServicesGroup().map(((el) => ({
          ...
          content: (): JSX.Element => (
            <div className="_px-2.5 _flex _flex-col _w-full _gap-6">
              <ServiceItem title={t(el.coll[0].title)} icon={el.coll[0].icon} t={t} onClick={() => setActive(true)} />
              <ServiceItem title={t(el.coll[1].title)} icon={el.coll[1].icon} t={t} onClick={() => setActive(true)} />
            </div>
          )
        })))}
      />
